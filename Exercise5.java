...
public class Exercise5 {
	...
	public static void main(String[] args) {
		...
		var totalsalaries = employees.stream().collect(
                Collectors.groupingBy(Employee::isFulltime, Collectors.summingDouble(Employee::getSalary)));
		System.err.println(totalsalaries);

		/*		
		Predicate<Employee> ifFullTime = employee -> employee.isFulltime(); 
		Predicate<Employee> ifPartTime = ifFullTime.negate();
		var totalSalaryToPartTimeEmployees =
		employees.stream()
				 .filter(ifPartTime)
				 .mapToDouble(Employee::getSalary)
				 .sum();		
		var totalSalaryToFullTimeEmployees =
				employees.stream()
				.filter(ifFullTime)
				.mapToDouble(Employee::getSalary)
				.sum();
		System.err.println("Full-time: "+totalSalaryToFullTimeEmployees+"\nPart-time: "+totalSalaryToPartTimeEmployees);
		*/
	}
}